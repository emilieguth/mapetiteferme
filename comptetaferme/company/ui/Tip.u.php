		...
		$h = '<div id="tip-wrapper" class="tip-wrapper-'.$navigation.'">';
			$h .= '<div class="tip-block">';
				...
				$h .= '<div class="tip-header">';
					...
					$h .= match($navigation) {
						'close' => '<a href="/company/tip?farm='.$eFarm['id'].'" class="btn btn-transparent btn-sm">'.\Asset::icon('caret-right-fill').' '.s("Toutes les astuces").'</a>',
						'next' => '<a href="/company/tip?farm='.$eFarm['id'].'" class="btn btn-transparent btn-sm">'.\Asset::icon('caret-right-fill').' '.s("Astuce suivante").'</a>',
						'inline' => ''
					};
				$h .= '</div>';
					...
					if($content['image']) {
						$h .= '<div class="tip-image"">';
							if($content['button'] !== NULL) {
								$h .= '<a href="/company/tip:click?id='.$tip.'&redirect='.urlencode($content['button'][0]).'" data-ajax-navigation="never">'.\Asset::image('farm', $tip.'.png').'</a>';
							} else {
				...
				$h .= '<div class="tip-button">';
					if($content['button'] !== NULL) {
						$h .= '<a href="/company/tip:click?id='.$tip.'&redirect='.urlencode($content['button'][0]).'" data-ajax-navigation="never" class="btn btn-outline-tip">'.$content['button'][1].'</a>';
					} else {
						$h .= '<span></span>';
					}
					if($navigation === 'close' or $navigation === 'inline') {
						$h .= '<a data-ajax="/company/tip:close?id='.$tip.'" data-ajax-method="get" class="btn btn-tip">'.\Asset::icon('x').' '.s("Ignorer").'</a>';
					}
				$h .= '</div>';
			$h .= '</div>';
		$h .= '</div>';
	...
	private function getContent(Farm $eFarm, string $fqn): array {
		...
		switch($fqn) {
			...
			case 'action-customize' :
				...
				$link = '/company/action:manage?farm='.$eFarm['id'];
			...
			case 'feature-time-disable' :
				...
				$link = '/company/farm:updateFeature?id='.$eFarm['id'];
			...
			case 'feature-tools' :
				...
				$link = '/company/tool:manage?farm='.$eFarm['id'];
