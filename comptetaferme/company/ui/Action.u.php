...
class ActionUi {
	...
	public function getManageTitle(\company\Company $eFarm): string {
		...
		$h = '<div class="util-action">';
			...
			$h .= '<div>';
				$h .= '<a href="/company/action:create?farm='.$eFarm['id'].'" class="btn btn-primary">'.\Asset::icon('plus-circle').' '.s("Nouvelle action").'</a>';
			$h .= '</div>';
	...
	public function getManage(\company\Company $eFarm, \Collection $cAction, \Collection $cCategory): string {
		...
		$h = '<div class="util-block-gradient">';
			$h .= '<p>';
				...
				$h .= '</p>';
				$h .= '<a href="/company/category:manage?farm='.$eFarm['id'].'" class="btn btn-outline-secondary">'.s("Personnaliser les catégories ").'</a>';
		$h .= '</div>';
		...
		$h .= '<table class="tbody-even tbody-bordered">';
			...
			foreach($cAction as $eAction) {
				...
				$h .= '<tbody>';
					...
					$h .= '<tr>';
						$h .= '<td>';
							$h .= \company\ActionUi::getColorCircle($eAction);
							$h .= encode($eAction['name']);
								...
								foreach($eAction['cMethod'] as $eMethod) {
									...
									$h .= '<div class="dropdown-list">';
										$h .= '<div class="dropdown-title">'.encode($eMethod['name']).'</div>';
											...
											$h .= '<a data-ajax="/company/method:doDelete" data-confirm="'.s("Supprimer définitivement cette méthode ?").'" post-id="'.$eMethod['id'].'" class="dropdown-item">'.s("Supprimer la méthode").'</a>';

									$h .= '</div>';
								}
								$h .= '<a href="/company/method:create?action='.$eAction['id'].$methodHelp.'" class="btn btn-sm btn-outline-primary">'.\Asset::icon('plus').'</a>';
							$h .= '</div>';
						$h .= '</td>';
						...
						$h .= '<td class="text-end" style="white-space: nowrap">';
							...
							$h .= '<div class="dropdown-list">';
								$h .= '<div class="dropdown-title">'.encode($eAction['name']).'</div>';
									...
									$h .= '<a href="/company/action:update?id='.$eAction['id'].'" class="dropdown-item">'.s("Modifier l'intervention").'</a>';
									$h .= '<a href="/company/method:create?action='.$eAction['id'].$methodHelp.'" class="dropdown-item">'.s("Ajouter une méthode de travail").'</a>';

									$h .= '<div class="dropdown-divider"></div>';

									if($eAction['fqn'] === NULL) {
										$h .= '<a data-ajax="/company/action:doDelete" data-confirm="'.s("Supprimer cette intervention ?").'" post-id="'.$eAction['id'].'" class="dropdown-item">';
	...
	public function create(\company\Company $eFarm, \Collection $cCategory): \Panel {
		...
		$h = $form->openAjax('/company/action:doCreate');
