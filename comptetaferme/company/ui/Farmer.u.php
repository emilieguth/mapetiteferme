...
class FarmerUi {
	...
	public static function urlManage(Farm $eFarm): string {
		return '/company/farmer:manage?farm='.$eFarm['id'].'';
	}
	...
	public function createUser(\company\Company $eFarm): \Panel {

		$form = new \util\FormUi();

		$h = $form->openAjax('/company/farmer:doCreateUser');
	...
	public function updateUser(\company\Company $eFarm, \user\User $eUser): \Panel {

		$form = new \util\FormUi();

		$h = $form->openAjax('/company/farmer:doUpdateUser');
	...
	public function getManageTitle(Farm $eFarm): string {
		...
		$h = '<div class="util-action">';
			...
			$h .= '<div>';
				$h .= '<a href="/company/farmer:create?farm='.$eFarm['id'].'" class="btn btn-primary">'.\Asset::icon('plus-circle').' '.s("Inviter un utilisateur dans l'équipe").'</a>';
			$h .= '</div>';
	...
	public function getManage(Farm $eFarm, \Collection $cFarmer, \Collection $cFarmerInvite, \Collection $cFarmerGhost): string {
		...
		if($cFarmer->notEmpty()) {
			...
			$h .= '<div class="util-buttons">';
				...
				foreach($cFarmer as $eFarmer) {
					...
					$h .= '<a href="/company/farmer:show?id='.$eFarmer['id'].'" class="util-button bg-secondary">';
						$h .= '<div>';
							$h .= '<h4>';
					...
					foreach($cFarmerInvite as $eFarmer) {
						...
						$h .= '<div class="util-button bg-primary">';
							$h .= '<div>';
								...
								$h .= '</div>';
								$h .= '<div class="mt-1">';
									$h .= '<a data-ajax="/company/farmer:doDeleteInvite" post-id="'.$eFarmer['id'].'" class="btn btn-transparent">';
										$h .= s("Supprimer");
									$h .= '</a> ';
									$h .= '<a data-ajax="/company/invite:doExtends" post-id="'.$eFarmer['invite']['id'].'" data-confirm="'.s("Voulez-vous vraiment renvoyer un mail d'invitation à cette personne ?").'" class="btn btn-transparent">';
		...
		if($cFarmerGhost->empty()) {
			...
			$h .= '<a href="/company/farmer:createUser?farm='.$eFarm['id'].'" class="btn btn-secondary">'.s("Créer un premier utilisateur fantôme").'</a>';

		} else {

			$h .= '<div class="h-line">';
				$h .= '<h2>'.\Asset::icon('snapchat').' '.s("Les utilisateurs fantômes").'</h2>';
				$h .= '<a href="/company/farmer:createUser?farm='.$eFarm['id'].'" class="btn btn-outline-primary">'.\Asset::icon('plus-circle').' '.s("Créer un utilisateur fantôme").'</a>';
			$h .= '</div>';
					...
					foreach($cFarmerGhost as $eFarmer) {
						...
						$h .= '<tr>';
							...
							$h .= '<td class="text-end">';
								...
								if($eFarmer['status'] === Farmer::OUT) {
									$h .= '<a data-ajax="/company/farmer:doUpdateStatus" post-id="'.$eFarmer['id'].'" post-status="'.Farmer::IN.'" class="btn btn-secondary">';
										$h .= \Asset::icon('arrow-up').' '.s("Intégrer à l'équipe");
									$h .= '</a> ';
								}

								$h .= '<a href="/company/farmer:updateUser?farm='.$eFarm['id'].'&user='.$eFarmer['user']['id'].'" class="btn btn-secondary">';
									$h .= \Asset::icon('gear-fill');
								$h .= '</a>';
								$h .= ' ';

								$h .= match($eFarmer['status']) {
									Farmer::OUT => '<a data-ajax="/company/farmer:doDeleteUser" post-farm="'.$eFarm['id'].'" post-user="'.$eFarmer['user']['id'].'" class="btn btn-danger" data-confirm="'.s("Cette action est irréversible, voulez-vous réellement supprimer cet utilisateur fantôme ?").'">'.\Asset::icon('trash').'</a>',
	...
	public function getUserTitle(\company\Farmer $eFarmer): string {
	...
	public function getUser(\company\Farmer $eFarmer, \Collection $cPresence, \Collection $cAbsence): string {
		...
		if($eFarmer['status'] === Farmer::OUT) {
			...
			$h .= '<div>';
				...
				if($eFarmer->canUpdateRole()) {
					...
					$h .= '<a href="/company/farmer:update?id='.$eFarmer['id'].'" class="btn btn-primary">';
				...
				if($eFarmer['farmGhost']) {
					$h .= '<a href="/company/farmer:create?farm='.$eFarmer['farm']['id'].'&farmer='.$eFarmer['id'].'" class="btn btn-primary">';
						$h .= s("Inviter à créer un compte");
					$h .= '</a> ';
					$h .= '<a data-ajax="/company/farmer:doUpdateStatus" post-id="'.$eFarmer['id'].'" post-status="'.Farmer::OUT.'" class="btn btn-primary">';
						$h .= s("Sortir de l'équipe");
					$h .= '</a> ';
				} else {

					if($eFarmer['user']->notEmpty()) {
						$h .= '<a data-ajax="/company/farmer:doDelete" post-id="'.$eFarmer['id'].'" class="btn btn-primary" data-confirm="'.s("Souhaitez-vous réellement retirer cet utilisateur de la ferme ?").'">';
							$h .= s("Sortir de l'équipe");
						$h .= '</a>';
					}
	...
	protected function createForm(Farmer $eFarmer, Farmer $eFarmerLink, string $origin): string {
		...
		$h = $form->openAjax('/company/farmer:doCreate', ['data-ajax-origin' => $origin]);
	...
	public function update(\company\Farmer $eFarmer): \Panel {

		$form = new \util\FormUi();

		$h = $form->openAjax('/company/farmer:doUpdate');
