...
class ToolUi {
	...
	public static function urlManage(Farm $eFarm, ?string $routineName = NULL): string {
		...
		return '/company/tool:manage?farm='.$eFarm['id'].''.($routineName ? '&routineName='.$routineName : '');
	...
	public function query(\PropertyDescriber $d, bool $multiple = FALSE) {
		...
		$d->autocompleteUrl = '/company/tool:query';
	...
	public function getList(\Collection $cTool): string {
		...
		$h = '<div class="tool-list">';
			foreach($cTool as $eTool) {
				$h .= '<a href="'.\company\ToolUi::url($eTool).'" class="tool-list-item">';
					$h .= \company\ToolUi::getVignette($eTool, '2.7rem', '1.8rem');
					$h .= encode($eTool['name']);
				$h .= '</a>';
			}
		$h .= '</div>';
	...
	public function getManageTitle(\company\Company $eFarm, ?string $routineName, array $tools, \Collection $cTool, \Search $search): string {
		...
		if($this->isEmpty($tools, $cTool, $search)) {
			...
			$h = '<div class="util-action">';
				...
				$h .= '<div>';
					...
					$h .= '<a href="/company/tool:create?farm='.$eFarm['id'].''.($routineName ? '&routineName='.$routineName : '').'" class="btn btn-primary">'.\Asset::icon('plus-circle').' '.($routineName ? RoutineUi::getProperty($routineName, 'createButton') : s("Nouveau matériel")).'</a>';
				$h .= '</div>';
			$h .= '</div>';
	...
	public function getManage(\company\Company $eFarm, ?string $routineName, array $tools, \Collection $cTool, Tool $eToolNew, \Collection $cActionUsed, \Search $search): string {
								...
								$h .= '<td class="text-end">';
									...
									$h .= '<div class="dropdown-list">';
										$h .= '<div class="dropdown-title">'.encode($eTool['name']).'</div>';
											...
											$h .= '<a href="/company/tool:update?id='.$eTool['id'].'" class="dropdown-item">';
												$h .= ($routineName ? RoutineUi::getProperty($routineName, 'dropdownUpdate') : s("Modifier le matériel"));
											$h .= '</a> ';

											$h .= match($eTool['status']) {

												Tool::ACTIVE => '<a data-ajax="/company/tool:doUpdateStatus" post-id="'.$eTool['id'].'" post-status="'.Tool::INACTIVE.'" class="dropdown-item">'.($routineName ? RoutineUi::getProperty($routineName, 'dropdownDisable') : s("Désactiver le matériel")).'</a>',
												Tool::INACTIVE => '<a data-ajax="/company/tool:doUpdateStatus" post-id="'.$eTool['id'].'" post-status="'.Tool::ACTIVE.'" class="dropdown-item">'.($routineName ? RoutineUi::getProperty($routineName, 'dropdownEnable') : s("Réactiver le matériel")).'</a>'

											};

											$h .= '<div class="dropdown-divider"></div>';

											$h .= '<a data-ajax="/company/tool:doDelete" data-confirm="'.($routineName ? RoutineUi::getProperty($routineName, 'deleteConfirm') : s("Supprimer ce matériel ?")).'" post-id="'.$eTool['id'].'" class="dropdown-item">';
	...
	public function getSearch(\company\Company $eFarm, \Collection $cActionUsed, \Search $search): string {

		$h = '<div id="tool-search" class="util-block-search stick-xs '.($search->empty(['status']) ? 'hide' : '').'">';

			$form = new \util\FormUi();

			$h .= $form->openAjax('/company/tool:manage', ['method' => 'get', 'id' => 'form-search']);
	...
	public function createForm(Tool $eTool, string $origin): string {
		...
		$h = $form->openAjax('/company/tool:doCreate', ['data-ajax-origin' => $origin]);
	...
	public function update(Tool $eTool, array $routines): \Panel {
		...
		$h = $form->openAjax('/company/tool:doUpdate');
