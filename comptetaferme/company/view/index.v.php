...
new AdaptativeView('planning', function($data, FarmTemplate $t) {
	...
	$t->tab = 'home';
	$t->subNav = (new \company\CompanyUi())->getPlanningSubNav($data->eFarm, $data->week);
	...
	switch($period) {
		...
		case \company\Employee::DAILY :
			...
			$t->footer = '';
			$t->canonical = \company\CompanyUi::urlPlanningDaily($data->eFarm, $data->week);

			$t->mainTitle = $uiTask->getWeekCalendar(
				$data->week,
				fn($week) => \company\CompanyUi::urlPlanningDaily($data->eFarm, $week)
			);
		...
		case \company\Employee::WEEKLY :
			...
			$t->mainTitle = $uiTask->getWeekCalendar(
				$data->week,
				fn($week) => \company\CompanyUi::urlPlanningWeekly($data->eFarm, $week),
		...
		case \company\Employee::YEARLY :
			...
			$t->canonical = \company\CompanyUi::urlPlanningYear($data->eFarm, $data->year, $data->month);
...
new AdaptativeView('/ferme/{id}/taches/{week}/{action}', function($data, FarmTemplate $t) {
	...
	$t->tab = 'home';
	$t->subNav = (new \company\CompanyUi())->getPlanningSubNav($data->eFarm);
...
new AdaptativeView('sequence', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlCultivationSequences($data->eFarm);
	$t->title = s("Itinéraires techniques de {value}", $data->eFarm['name']);
	$t->tab = 'cultivation';
	$t->subNav = (new \company\CompanyUi())->getCultivationSubNav($data->eFarm);
...
new AdaptativeView('series', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlCultivationSeries($data->eFarm, season: $data->season);
	...
	$t->mainYear = $uiFarm->getSeasonsTabs($data->eFarm, fn($season) => \company\CompanyUi::urlCultivationSeries($data->eFarm, season: $season), $data->season);
...
new AdaptativeView('mapEmpty', function($data, FarmTemplate $t) {
	...
	$t->mainYear .= (new \company\CompanyUi())->getSeasonsTabs($data->eFarm, fn($season) => \company\CompanyUi::urlSoil($data->eFarm, $season), $data->season);
	$t->mainTitle = '<h1>';
...
new AdaptativeView('cartography', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlCartography($data->eFarm, $data->season);
	$t->title = s("Plan de {value}", $data->eFarm['name']);

	$h = '<div class="util-action">';
		$h .= '<h1>';
			if($data->eFarm->canManage()) {
				$h .= '<a href="'.\company\CompanyUi::urlSettings($data->eFarm).'"  class="h-back">'.\Asset::icon('arrow-left').'</a>';
			}
	...
	$t->mainTitle = $h;
	$t->mainYear = (new \company\CompanyUi())->getSeasonsTabs($data->eFarm, fn($season) => \company\CompanyUi::urlCartography($data->eFarm, $season), $data->season);;
...
new AdaptativeView('soil', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlCultivationSoil($data->eFarm, season: $data->season);
	...
	$t->mainYear = $uiFarm->getSeasonsTabs($data->eFarm, fn($season) => \company\CompanyUi::urlCultivationSoil($data->eFarm, season: $season), $data->season);
	...
	if($data->cZone->notEmpty()) {
		...
		echo '<div class="util-block-help">';
			...
			echo '<a href="'.\company\CompanyUi::urlCartography($data->eFarm, $data->season).'" class="btn btn-secondary">'.s("Créer le plan de ma ferme").'</a>';
		echo '</div>';
...
new AdaptativeView('sellingSales', function($data, FarmTemplate $t) {
	...
	$t->tab = 'selling';
	$t->subNav = (new \company\CompanyUi())->getSellingSubNav($data->eFarm);

	$t->title = s("Ventes de {value}", $data->eFarm['name']);
	$t->canonical = \company\CompanyUi::urlSellingSalesAll($data->eFarm);
	...
	if(
		...
		echo '<div class="util-block-help">';
			...
			echo '<ul>';
				echo '<li>'.s("Référencez <link>votre gamme de produits</link>", ['link' => '<a href="'.\company\CompanyUi::urlSellingProduct($data->eFarm).'">']).'</li>';
				echo '<li>'.s("Créez les ventes de vos <link>clients particuliers et professionnels</link>", ['link' => '<a href="'.\company\CompanyUi::urlSellingCustomer($data->eFarm).'">']).'</li>';
				echo '<li>'.s("Éditez vos devis, bons de livraisons et factures au format PDF").'</li>';
				echo '<li>'.s("Ouvrez <link>des boutiques en ligne</link> avec en option le paiement par carte bancaire", ['link' => '<a href="'.\company\CompanyUi::urlShopList($data->eFarm).'">']).'</li>';
				echo '<li>'.s("Analysez vos ventes avec graphiques et statistiques").'</li>';
			echo '</ul>';
			echo '<p>'.s("Avant de créer votre première vente, regardez au préalable comment <customer>créer des clients</customer> et <items>référencer vos produits</items>. Une fois que c'est fait, c'est parti !", ['customer' => '<a href="'.\company\CompanyUi::urlSellingCustomer($data->eFarm).'">', 'items' => '<a href="'.\company\CompanyUi::urlSellingProduct($data->eFarm).'">']).'</p>';
		...
		$t->mainTitle = (new \company\CompanyUi())->getSellingSalesTitle($data->eFarm, Setting::get('main\viewSellingSales'));
...
new AdaptativeView('/ferme/{id}/clients', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlSellingCustomer($data->eFarm);
...
new AdaptativeView('/ferme/{id}/produits', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlSellingProduct($data->eFarm);
...
new AdaptativeView('/ferme/{id}/stocks', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlSellingStock($data->eFarm);
	...
	if($data->cProduct->empty()) {
		...
		echo '<div class="util-block-help">';
			...
			echo '<a href="'.\company\CompanyUi::urlSellingProduct($data->eFarm).'" class="btn btn-secondary">'.s("Voir mes produits").'</a>';
		echo '</div>';
...
new AdaptativeView('/ferme/{id}/boutiques', function($data, FarmTemplate $t) {
	...
	$t->tab = 'shop';
	$t->subNav = (new \company\CompanyUi())->getShopSubNav($data->eFarm);

	$t->title = s("Boutiques de {value}", $data->eFarm['name']);
	$t->canonical = \company\CompanyUi::urlShopList($data->eFarm);
...
new AdaptativeView('/ferme/{id}/catalogues', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlShopCatalog($data->eFarm);
...
new AdaptativeView('/ferme/{id}/livraison', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlShopPoint($data->eFarm);
...
new AdaptativeView('/ferme/{id}/factures', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlSellingSalesInvoice($data->eFarm);
	...
	if($data->hasInvoices === FALSE) {
		...
		echo '<div class="util-block-help">';
			...
			if($data->hasSales) {
				...
				echo '<a href="'.\company\CompanyUi::urlSellingSalesAll($data->eFarm).'" class="btn btn-secondary">'.s("Retourner sur les ventes").'</a>';
			}
		echo '</div>';
		echo '<div class="util-block-help">';
...
new AdaptativeView('/ferme/{id}/etiquettes', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlSellingSalesLabel($data->eFarm);
...
new AdaptativeView('analyzeReport', function($data, FarmTemplate $t) {
	...
	$t->tab = 'analyze';
	$t->subNav = (new \company\CompanyUi())->getAnalyzeSubNav($data->eFarm);
...
new AdaptativeView('analyzeWorkingTime', function($data, FarmTemplate $t) {
	...
	if($data->years === []) {
		...
		$t->mainYear = (new \company\AnalyzeUi())->getYears($data->eFarm, $data->years, $data->year, $data->month, $data->week, $data->category);
		$t->mainTitle = (new \company\CompanyUi())->getAnalyzeWorkingTimeTitle($data->eFarm, $data->years, $data->year, $data->month, $data->week, $data->category);
...
new AdaptativeView('analyzeSelling', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlAnalyzeSelling($data->eFarm, $data->year, $data->category);
...
new AdaptativeView('analyzeCultivation', function($data, FarmTemplate $t) {
	...
	$t->canonical = \company\CompanyUi::urlAnalyzeCultivation($data->eFarm, $data->season, $data->category);
...
new AdaptativeView('/ferme/{id}/configuration', function($data, FarmTemplate $t) {
	...
	echo (new \company\CompanyUi())->getSettings($data->eFarm, $data->eNews);
